...
class MessagesRepository {
  ...
  Future<List<Conversation>> fetchConversations(String userId,
    ...
    try {
      ...
      await _hydrateGenericFilePreviews(ids, lastMsgMap);
  ...
  Future<List<Map<String, dynamic>>> _fetchLastMessageRows(
    ...
    try {
      final rows = await supabase
          .from('messages')
          .select(
              'conversation_id, content, media_type, media_url, media_file_name, metadata, original_post_id, is_deleted, created_at')
  ...
  Future<void> _hydrateGenericFilePreviews(
    List<String> ids,
    Map<String, String?> lastMsgMap,
  ) async {
    final targets = ids.where((id) {
      final value = lastMsgMap[id]?.trim().toLowerCase();
      return value == 'fayl' || value == 'file' || value == 'document';
    }).toList(growable: false);
    if (targets.isEmpty) return;

    final hydrated = await _fetchLastMessagesWithoutRpc(targets);
    for (final entry in hydrated.entries) {
      final value = entry.value?.trim();
      if (value == null || value.isEmpty) continue;
      final lower = value.toLowerCase();
      if (lower == 'fayl' || lower == 'file' || lower == 'document') continue;
      lastMsgMap[entry.key] = value;
    }
  }
  ...
  String? _attachmentFileNameFromMap(Map<String, dynamic> map) {
    ...
    try {
      ...
      // Keep raw basename when malformed percent-encoding is present.
    }
    name = name.replaceFirst(
      RegExp(r'^\d{10,}-[a-z0-9]{6,12}-', caseSensitive: false),
      '',
    );
    return RegExp(r'\.[a-z0-9]{1,10}$', caseSensitive: false).hasMatch(name)
        ? name
        : null;
  }

  static String? _attachmentFileNameFromMessage(Message message) {
    ...
    if (content != null &&
        content.isNotEmpty &&
        RegExp(r'\.[a-z0-9]{1,10}$', caseSensitive: false).hasMatch(content)) {
      return content;
    }
    return null;
  }
  ...
  String? _messagePreviewFromMap(Map<String, dynamic> map) {
    ...
    if (type == 'file' || type == 'document') {
      final fileName = _attachmentFileNameFromMap(map);
      ...
      if (fileContent != null &&
          ...
          !const {'fayl', 'file', 'document'}
              .contains(fileContent.toLowerCase())) {
        return fileContent;
      }
      return 'Fayl';
    }
  ...
  static String? previewForMessage(Message message) {
    ...
    final mediaLabel = _mediaPreviewLabel(type);
    if (type == 'file' || type == 'document') {
      final fileName = _attachmentFileNameFromMessage(message);
      if (fileName != null && fileName.isNotEmpty) return fileName;

      final fileContent = _normalizePreviewText(message.content, mediaLabel);
      if (fileContent != null &&
          fileContent.isNotEmpty &&
          !const {'fayl', 'file', 'document'}
              .contains(fileContent.toLowerCase())) {
