...
class LiveWebRtcService {
  ...
  Future<void> connect() async {
    ...
    _channel!
        ...
        .onPresenceSync((payload) {
          ...
          final peerIds = _channel!.presenceState()
              .map((p) => p.key)
  ...
  Future<void> _handleOffer(Map<String, dynamic> payload) async {
    ...
    for (final candidate in _pending.remove(from) ?? const <RTCIceCandidate>[]) {
  ...
  Future<void> _handleIce(Map<String, dynamic> payload) async {
    ...
    final pc = _peers[from];
    if (pc == null) {
      _pending[from] = (_pending[from] ?? <RTCIceCandidate>[])..add(candidate);
      return;
    }
    try {
      await pc.addCandidate(candidate);
