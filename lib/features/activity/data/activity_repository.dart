...
class ActivityRepository {
  ...
  Future<ActivitySummary?> fetchSummary(String userId) async {
    ...
    for (final raw in (logs as List)) {
      ...
      final logDate = DateTime.tryParse((log['created_at'] as String?) ?? '')?.toLocal();
      ...
      thisYear += minutes;
      if (!logDate.isBefore(monthStart)) thisMonth += minutes;
      if (!logDate.isBefore(weekStart)) thisWeek += minutes;
      if (!logDate.isBefore(todayStart)) today += minutes;
  ...
  Future<void> logActivity(String userId, String page, int durationSeconds) async {
