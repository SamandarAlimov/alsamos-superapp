...
class NotificationsRepository extends BaseRepository {
  ...
  Future<void> markAsRead(String id) async {
    return guard('markAsRead', () async {
      await _db.table('notifications').update({'is_read': true}).eq('id', id);
    });
  }

  Future<void> markAllAsRead(String userId) async {
    return guard('markAllAsRead', () async {
      await _db
          .table('notifications')
          .update({'is_read': true})
          .eq('user_id', userId)
  ...
  Future<void> delete(String id) async {
    return guard('delete', () async {
      await _db.table('notifications').delete().eq('id', id);
    });
  }

  Future<void> respondToCollaborationInvite({
    required String collaborationId,
    required bool accept,
  }) async {
    return guard('respondToCollaborationInvite', () async {
      await _db.table('post_collaborators').update({
