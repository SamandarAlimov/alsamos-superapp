...
class EditProfileDialog extends ConsumerStatefulWidget {
  ...
  static Future<void> show(BuildContext context, FullProfile profile) =>
      showDialog(context: context, builder: (_) => EditProfileDialog(profile: profile));
...
class _EditProfileDialogState extends ConsumerState<EditProfileDialog> {
  ...
  Future<void> _pickAndUpload({required bool isCover}) async {
    ...
    setState(() {
      if (isCover) { _uploadingCover = true; } else { _uploadingAvatar = true; }
    });
    try {
      final bucket = isCover ? 'covers' : 'avatars';
      final ext = picked.path.split('.').last.toLowerCase();
      final path = '${widget.profile.id}/${DateTime.now().millisecondsSinceEpoch}.$ext';
      ...
      if (!mounted) return;
      setState(() {
        if (isCover) { _coverUrl = publicUrl; } else { _avatarUrl = publicUrl; }
      });
    } catch (e) {
      if (!mounted) return;
      ...
      if (mounted) {
        setState(() {
        if (isCover) { _uploadingCover = false; } else { _uploadingAvatar = false; }
      });
      }
    }
  }
  ...
  Future<void> _save() async {
    ...
    if (username.isEmpty) {
      setState(() => _error = 'Username bo\'sh bo\'lmasligi kerak');
      return;
    }
    if (!RegExp(r'^[a-z0-9_]{3,20}$').hasMatch(username)) {
      setState(() => _error = 'Username 3-20 ta belgi, faqat a-z 0-9 _');
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _saving = false; _error = 'Saqlash xatosi: $e'; });
    }
  }
        ...
        child: SingleChildScrollView(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
            ...
            Stack(children: [
                ...
                child: Container(
                  ...
                  child: _coverUrl != null
                      ...
                      : Container(decoration: BoxDecoration(gradient: LinearGradient(
                          colors: [primary.withValues(alpha: 0.6), primary.withValues(alpha: 0.2)],
                          begin: Alignment.topLeft, end: Alignment.bottomRight))),
                ),
              ),
              Positioned(right: 10, top: 10, child: Material(
                color: Colors.black.withValues(alpha: 0.5), shape: const CircleBorder(),
                child: InkWell(
                  customBorder: const CircleBorder(),
                  onTap: _uploadingCover ? null : () => _pickAndUpload(isCover: true),
                  child: Padding(
              ...
              Positioned(left: 16, bottom: -32, child: Stack(children: [
                Container(
                  width: 80, height: 80,
                  ...
                  child: ClipOval(child: UserAvatar(avatarUrl: _avatarUrl, size: 72, fallback: widget.profile.initial)),
                ),
                Positioned(right: 0, bottom: 0, child: Material(
                  color: primary, shape: const CircleBorder(),
                  child: InkWell(
                    customBorder: const CircleBorder(),
                    onTap: _uploadingAvatar ? null : () => _pickAndUpload(isCover: false),
                    child: Padding(
              ...
              Positioned(right: 6, top: 6, child: Material(
            ...
            Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(height: 12),
                _label(c, "Bio"),
                _field(c, _bio, hint: "O'zingiz haqida...", maxLines: 3, maxLength: 160),
                const SizedBox(height: 12),
                _label(c, "Veb-sayt"),
  ...
  Widget _label(AlsamosColors c, String text) => Padding(
    ...
    child: Text(text, style: TextStyle(color: c.mutedForeground, fontSize: 12, fontWeight: FontWeight.w500)),
  );
