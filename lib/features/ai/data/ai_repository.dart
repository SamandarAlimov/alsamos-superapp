...
class AiRepository {
  ...
  Stream<String> streamChat(
      List<AiMessage> messages, String userId, {String? contextInfo}) async* {
    final uri = Uri.parse('${ApiConstants.supabaseUrl}/functions/v1/ai-assistant');
    final aiMessages = messages.map((m) => {'role': m.role, 'content': m.content}).toList();
    if (contextInfo != null && contextInfo.isNotEmpty && aiMessages.isNotEmpty) {
      aiMessages[0] = {
        ...aiMessages[0],
      ...
      await for (final chunk in resp.stream.transform(utf8.decoder)) {
        ...
        while ((nl = buffer.indexOf('\n')) != -1) {
          ...
          try {
            ...
            final delta = (parsed['choices'] as List?)?.first?['delta']?['content'];
  ...
  Future<({String imageUrl, String? revisedPrompt})> generateImage(String prompt) async {
    ...
    if (data is Map) {
      ...
      final rev = (data['revised_prompt'] ?? data['revisedPrompt']) as String?;
  ...
  Future<({String id, String? content, String? authorName, String? mediaUrl})?>
    ...
    try {
      ...
      String? firstMedia;
      if (media is List && media.isNotEmpty) firstMedia = media.first?.toString();
      return (
        id: res['id'] as String,
        content: res['content'] as String?,
        authorName: (profile?['display_name'] ?? profile?['username']) as String?,
        mediaUrl: firstMedia,
      );
    } catch (_) {
      return null;
