...
class _ChannelDetailPageState extends ConsumerState<ChannelDetailPage> {
  ...
  Future<void> _load() async {
    try {
      final r = await _client.from('channel_posts')
          .select('id, content, media_urls, likes_count, comments_count, views_count, created_at')
    ...
    return Scaffold(
      ...
      body: CustomScrollView(slivers: [
        SliverAppBar(
          ...
          title: Text(ch.name, style: TextStyle(color: c.foreground, fontSize: 16, fontWeight: FontWeight.w700)),
                ...
                : Container(
                    decoration: BoxDecoration(
                      gradient: LinearGradient(
                        ...
                        colors: [primary.withValues(alpha: 0.5), primary.withValues(alpha: 0.2)],
                      ),
                    ),
                  ),
          ),
              ...
              Row(children: [
                CircleAvatar(
                  ...
                  child: (ch.avatarUrl == null || ch.avatarUrl!.isEmpty)
                      ? Text(ch.name[0].toUpperCase(),
                          style: TextStyle(color: primary, fontWeight: FontWeight.w700, fontSize: 22))
                      : null,
                ),
                const SizedBox(width: 14),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(ch.name, style: TextStyle(color: c.foreground, fontSize: 18, fontWeight: FontWeight.w700)),
                  if (ch.username != null)
                    Text('@${ch.username}', style: TextStyle(color: c.mutedForeground, fontSize: 13)),
                  ...
                  Row(children: [
                    ...
                    Icon(LucideIcons.fileText, size: 14, color: c.mutedForeground),
                    const SizedBox(width: 4),
                ...
                if (!isMine)
                  FilledButton.icon(
                    ...
                    icon: Icon(ch.isMember ? LucideIcons.check : LucideIcons.userPlus, size: 16),
                    label: Text(ch.isMember ? 'Obuna' : 'Obuna bo\'lish'),
                    style: FilledButton.styleFrom(
                      backgroundColor: ch.isMember ? c.muted : primary,
                      foregroundColor: ch.isMember ? c.foreground : Colors.white,
                    ),
                  ),
              ]),
              if (ch.description != null && ch.description!.isNotEmpty) ...[
                const SizedBox(height: 14),
                Text(ch.description!, style: TextStyle(color: c.foreground.withValues(alpha: 0.85), fontSize: 14, height: 1.4)),
              ],
              const SizedBox(height: 14),
        ...
        if (_loading)
          ...
          SliverFillRemaining(child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
            Icon(LucideIcons.fileText, size: 56, color: c.mutedForeground.withValues(alpha: 0.3)),
            const SizedBox(height: 12),
            Text('Hozircha postlar yo\'q', style: TextStyle(color: c.mutedForeground, fontSize: 14)),
          ])))
        else
                ...
                child: Container(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    if ((p['content'] as String?)?.isNotEmpty ?? false)
                      Text(p['content'] as String, style: TextStyle(color: c.foreground, fontSize: 14, height: 1.4)),
                    if (media.isNotEmpty) ...[
                      ...
                      ClipRRect(
                        ...
                        child: AspectRatio(
                          aspectRatio: 16 / 9,
                          child: CachedNetworkImage(imageUrl: media.first, fit: BoxFit.cover),
                        ),
                      ),
                    ],
                    const SizedBox(height: 10),
                    Row(children: [
                      Icon(LucideIcons.heart, size: 14, color: c.mutedForeground),
                      const SizedBox(width: 4),
                      Text(_fmt((p['likes_count'] as num?)?.toInt() ?? 0),
                          style: TextStyle(color: c.mutedForeground, fontSize: 12)),
                      const SizedBox(width: 14),
                      Icon(LucideIcons.messageCircle, size: 14, color: c.mutedForeground),
                      const SizedBox(width: 4),
                      Text(_fmt((p['comments_count'] as num?)?.toInt() ?? 0),
                          style: TextStyle(color: c.mutedForeground, fontSize: 12)),
                      const SizedBox(width: 14),
